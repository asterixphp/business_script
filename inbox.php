...
$del=mysql_query("update mlm_outbox set outbox_tostatus='1' where outbox_id='$delid'");
...
$rsts=mysql_query("update mlm_outbox set outbox_readstatus='1' where outbox_id='$readsts'");
...
<link href="css/B_red.css" rel="stylesheet" type="text/css" />
    ...
    <body>
		...
		$url='?';
									...
									$inb=mysql_query("select * from mlm_outbox where (outbox_toupid ='$_SESSION[userid]' and outbox_status='0') and (outbox_usertype='2' and outbox_tostatus='0') order by outbox_id desc LIMIT {$startpoint} , {$limit}");
									$nom_rows=mysql_num_rows(mysql_query("select * from mlm_outbox where (outbox_toupid ='$_SESSION[userid]' and outbox_status='0') and (outbox_usertype='2' and outbox_tostatus='0')"));
									...
									{	
									$useinb=mysql_fetch_array(mysql_query("select * from mlm_register where user_id='$rinb[outbox_userid]'"));
