...
$del=mysql_query("update mlm_outbox set outbox_fromstatus='1' where outbox_id='$delid'");
...
<link href="css/B_red.css" rel="stylesheet" type="text/css" />
    ...
    <body>
		...
		$url='?';
									...
									$out=mysql_query("select * from mlm_outbox where (outbox_userid ='$_SESSION[userid]' and outbox_status='0') and (outbox_usertype!='0' and outbox_fromstatus='0') order by outbox_id desc LIMIT {$startpoint} , {$limit}");
									$nom_rows=mysql_num_rows(mysql_query("select * from mlm_outbox where (outbox_userid='$_SESSION[userid]' and outbox_status='0') and (outbox_usertype!='0' and outbox_fromstatus='0')"));
