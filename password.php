...
$menu7='class="active"';
$sel_login=mysql_fetch_array(mysql_query("SELECT * FROM mlm_admin WHERE admin_id='$_SESSION[admin_id]'"));
 //echo "SELECT * FROM skill_admin WHERE admin_id='$_SESSION[admin_id]'";
 //echo $sel_login['adminpassword'];
if(isset($_REQUEST['submit']))
{
 ...
 $admin_id=$_SESSION['admin_id'];
	...
	//echo $cur_pass.", ".$current_pass.", ".$new_pass.", ".$con_new_pass;exit;
	...
	{
		//echo "ok<br>";
		//echo "SELECT * FROM skill_admin WHERE admin_id='$admin_id' AND adminpassword='$current_pass'";
		$check = mysql_num_rows(mysql_query("SELECT * FROM mlm_admin WHERE admin_id='$admin_id' AND admin_password='$current_pass'"));
		//echo $check;
		if($check == 1)
		{
			$update = mysql_query("UPDATE mlm_admin SET admin_username='$username',admin_password='$new_pass' WHERE admin_id='$admin_id'");
			if($update)
			{
...
-->							<!--	<input type="submit" name="submit" value="SUBMIT" class="btn btn-info" style="font-weight:bold;">-->
								<input type="button"  value="SUBMIT" class="btn btn-info" style="font-weight:bold;" onclick="alert('This feature is disabled for demo version');" disabled="disabled">		
